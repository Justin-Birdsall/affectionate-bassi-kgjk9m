...
import { deathAnimation } from "./deathanm.js";
import { idleAnimation } from "./idle_anim.js";
import { ratAnimation } from "./center_rat.js";
...
(async () => {
  ...
  const defaultIcon = "url(src/assets/cheshire.png), auto";
  app.renderer.events.cursorStyles.default = defaultIcon;
  const movementSpeed = 0.05;
  await Assets.load("src/assets/center_rat.json");
  await Assets.load("src/assets/rat.json");
  const death_anim = deathAnimation(app); // Call the deathAnimation function
  const idle_anim = idleAnimation(app);
  const rat_anim = ratAnimation(app);
  // Strength of the impulse push between two objects
  const impulsePower = 5;
  app.stage.addChild(death_anim);
  app.stage.addChild(idle_anim);
  app.stage.addChild(rat_anim);
  // Test For Hit
  ...
  const redSquare = new Sprite(idle_anim);
  ...
  app.stage.on("mousemove", (event) => {
    mouseCoords.x = event.global.x - 10;
    mouseCoords.y = event.global.y - 10;
  });
